...
Schema.getDocusaurusMetadata = (filePath, schema) => {
  let temp = path.basename(filePath, path.extname(filePath));
  // TODO should err be thrown here?
  var rawMetadata = temp.split('.')[0].match(/[A-Z]{1,20}[a-z]+/g).toString().replace(",Event", "").split(",").join(" ");
  schema.docusaurusPageId = temp.split('.')[0];
  schema.docusaurusSidebarLabel = rawMetadata;
  schema.docusaurusPageTitle = rawMetadata;
  return schema;
};
...
Schema.process = (schemaMap, schemaPath, docDir, schemaDir,
  ...
  return Promise.mapSeries(keys, (schemaKey) => {
    ...
    return Schema.getExamples(schemaMap[schemaKey].filePath, schema)
      ...
      .then(egsSchema => Schema.getDocusaurusMetadata(schemaMap[schemaKey].filePath, egsSchema))
      .then((allSchema) => {
